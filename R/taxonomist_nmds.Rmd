...
* Do an NMDS on the phyto data using taxonomists as sites.
* Convert phyto07 to presense absence
* summarize by taxonomist
* run the NMDS

## data steps
* get the phyto07, taxonomist and env data
* join the taxonomist and phyto07 data
...
tax<-inner_join(taxonomist,phyto07)%>%select(-site_id) #1148  255
tax1<-tax
tax1[,-1][tax1[,-1]>0]<-1

table(tax$Microcystis>0)  #true=661
sum(tax1$Microcystis)  #661

tax<-tax %>% group_by(taxonomist) %>% summarise_all(funs(sum))
tax1<-tax1 %>% group_by(taxonomist) %>% summarise_all(funs(sum))
```

```{r nmds1, include=FALSE, echo=FALSE, cache=FALSE,eval=TRUE} 
nms_tax1<-metaMDS(tax1[,-1],k=4,trymax=10000,parallel=32)
save(nms_tax1,file=here("data/nms_tax1.rda"))
```
