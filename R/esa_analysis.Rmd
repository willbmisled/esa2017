...
save(phyto07,file=here('data/phyto07.rda'))
...
a<-as.data.frame(table(env$cyl_tox_cat))
ct<-data.frame(toxin='cylindrospermopsin',none=a$Freq[1],low=a$Freq[2],med=a$Freq[3],high=a$Freq[4])
a<-as.data.frame(table(env$mic_tox_cat))
mt<-data.frame(toxin='microcystin',none=a$Freq[1],low=a$Freq[2],med=a$Freq[3],high=a$Freq[4])
a<-as.data.frame(table(env$sax_tox_cat))
st<-data.frame(toxin='saxitoxin',none=a$Freq[1],low=a$Freq[2],med=a$Freq[3],high=a$Freq[4])
tt<-rbind(ct,mt,st)

write.csv(tt,file=here("output/tox_cat_table.csv"),row.names = FALSE)
```
...
#mic toxin
  qq<-plotLMH(cat=env$mic_tox_cat,target=c('mic_det','microcystin'),ef=ef07,scale=7,title='Microcystin Toxin Category',xnudge=c(0,0),ynudge=c(-.1,.1))
  qq
  ###output plot
...
#mic prod
  qq<-plotLMH(cat=env$mic_prod_abund_cat,target=c('mic_det','microcystin'),ef=ef07,scale=7,title='Potential Microcystin Producer Abundance',xnudge=c(0,0),ynudge=c(-.2,.1))
  qq
  ###output plot
...
#sax toxin
qq<-plotLMH(cat=env$sax_tox_cat,target=c('sax_det','saxitoxin'),ef=ef07,scale=7,title='Saxitoxin Category',xnudge=c(0,0),ynudge=c(.1,.1))
qq
###output plot
...
#sax prod
  qq<-plotLMH(cat=env$sax_prod_abund_cat,target=c('sax_det','saxitoxin'),ef=ef07,scale=7,title='Potential Saxitoxin Producer Abundance',xnudge=c(0,0),ynudge=c(-.2,.1))
  qq
  ###output plot
...
#cyano_abund
  qq<-plotLMH(cat=env$cyano_abund_cat,target=c('cyano_abund','cyl_det','mic_det','sax_det'),ef=ef07,scale=7,title='Cyanobacteria Abundance',xnudge=c(0,0,0,0),ynudge=c(.1,.1,-.1,-.1))
  qq
  ###output plot
...
#chla_cat
  qq<-plotLMH(cat=env$chla_cat,target=c('chla','cyl_det','mic_det','sax_det'),ef=ef07,scale=7,title='Cyanobacteria Abundance',xnudge=c(0,-.1,0,0),ynudge=c(.1,.1,-.1,.1))
  qq
  ###output plot
  jpeg(here("output/nms_grp_cyano_abund_cat.jpeg")) 
  qq
...
```{r rfcyano_present, include=FALSE, eval=FALSE}
load("C:/bryan/rscripts/esa2017bigdata/rf_grp2_cyano07.rda")
varImpPlot(rf_grp2_cyano07,type=2) #mean decrease in node impurity
varImpPlot(rf_grp2_cyano07,type=1) #mean decrease in accuracy
