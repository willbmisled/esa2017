...
phyto07<-select(phyto07,-taxonomist_cnt) #1,148 × 256
save(phyto07,taxonomist,file=here('data/phyto07.rda'))
...
rf <- ind_spp$relfrq;names(rf)<-c('relfreq1','relfreq2')
ra <- ind_spp$relabu;names(ra)<-c('relabund1','relabund2')
gr <- ind_spp$maxcls
iv <- ind_spp$indcls
pv <- ind_spp$pval
fr <- apply(phyto07[,-1]>0, 2, sum)
...
indvalsummary <- data.frame(sp=row.names(ind_spp),group=gr, indval=iv, pvalue=pv, freq=fr)
indvalsummary <- indvalsummary[order(indvalsummary$group, -indvalsummary$indval),]

head(rownames_to_column(indvalsummary))
head(indvalsummary)
