...
#sax toxin
qq<-plotLMH(cat=env$sax_tox_cat,target=c('sax_det','saxitoxin'),ef=ef07,scale=7,title='Saxitoxin Toxin Category',xnudge=c(0,0),ynudge=c(.1,.1))
qq
###output plot
...
* color lake points by "lake group"  ##################################
...
###output plot
jpeg(here("output/nms_grp_lake_group.jpeg")) 
qq
dev.off()
```
...
#add site points
qq<-gg+geom_point(aes(x=NMDS1,y=NMDS2,col=factor(chla_cat)),size=2) +
      ggtitle('Chlorophyll a Level')+
      theme(plot.title = element_text(lineheight=4.8, face="bold",hjust = 0.5))
qq
...
indvalsummary_tox<-inner_join(mutate(indvalsummary,genus=rownames(indvalsummary)),tox)
